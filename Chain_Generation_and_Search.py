...
def Chain_Search(Chain,K,alpha=0.5):
    ...
    if n <=K:
        ELPD_values = [ELPD(model) for model in Chain]
    ...
    # if number of models in the chain is strictly larger than K, then there is no choice but to conduct a search
        ...
        while num_ELPD_computed < K and cur_ind < n:
            ...
            if not (num_ELPD_computed ==0 or num_ELPD_computed==1):
                ...
                ELPD_slope_cur_iter = abs((ELPD(ELPD_computed_model_indices[-1])-ELPD(ELPD_computed_model_indices[-2])))/(ELPD_computed_model_indices[-1]-ELPD_computed_model_indices[-2])
                ELPD_slope_previous_iter = abs((ELPD(ELPD_computed_model_indices[-2])-ELPD(ELPD_computed_model_indices[-3])))/(ELPD_computed_model_indices[-2]-ELPD_computed_model_indices[-3])
